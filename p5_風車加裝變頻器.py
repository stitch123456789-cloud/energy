...
# 計算總台數與欄數 (1標題欄 + N台風扇 + 1合計欄)
num_fans = len(fan_list) # fan_list 是您計算過程中儲存每台數據的清單
...
for t in st.session_state.towers:
    ...
    for i in range(f_count):
        d = fan_list[col_ptr - 1 + i] # 對應計算好的數據
        ...
        # 統一格式 (標楷體、12號、置中)
        for r in range(2, 7): 
            fix_cell_font(table.cell(r, col_ptr + i), is_bold=(r==6))
    col_ptr += f_count
