...
# --- 2. 初始值與 Session State 邏輯 (放在最前面防止 NameError) ---
# 設定預設基準值
...
# --- 5. Word 生成與計算 ( build_word_table 內建過濾 None 功能 ) ---
def build_word_table(doc, op_df):
    # 過濾掉使用者誤增的空行
    op_df = op_df.dropna(subset=["RT", "台數", "效率(kW/RT)"])
...
old_desc = "、".join([f"{r['台數']}台{r['容量(RT)']}RT {r['型式']}" for _, r in old_cfg.iterrows() if r['台數'] is not None])
...
new_desc_word = " + ".join([f"{r['容量(RT)']}RT×{r['台數']}" for _, r in new_cfg.iterrows() if r['台數'] is not None])
...
# 抑制需量 (取夏季行計算)
summer_idx = 1
s_old = old_op.iloc[summer_idx]; s_new = new_op.iloc[summer_idx]
suppress_demand = (s_old['RT']*s_old['台數']*s_old['效率(kW/RT)']) - (s_new['RT']*s_new['台數']*s_new['效率(kW/RT)'])
